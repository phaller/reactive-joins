...
class RxReactBench extends PerformanceTest.OfflineReport {
  ...
  performance of "zipMap" config(
    ...
    using(sumSizes) curve("Us") in { size =>
      ...
      val result = join {
        case o1(x) && o2(y) => latch.countDown()
      ...
      result.subscribe((_: Unit) => (), (e: Throwable) => (), () => ())
      latch.await
    ...
    using(sumSizes) curve("ReactiveX") in { size =>
      ...
      val p1 = RxJoinObservable.from(o1).and(o2).then((x: Int, y: Int) => latch.countDown())
      val p2 = RxJoinObservable.from(o1).and(o3).then((x: Int, y: Int) => latch.countDown())
